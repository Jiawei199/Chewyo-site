...
( function( $, data, wp, ajaxurl ) {
	$( function() {
		var $table          = $( '.wc-shipping-zone-methods' ),
			...
			// Do not use WC core class `wc-shipping-zone-method-save` — core binds AJAX to
			// `woocommerce_shipping_zone_methods_save_changes`; we use `ep_shipping_zone_methods_save_changes`.
			$save_button    = $( '.ep-easyparcel-shipping-zone-method-save' ),
			$row_template   = wp.template( 'wc-shipping-zone-method-row' ),
			$blank_template = wp.template( 'wc-shipping-zone-method-row-blank' ),
			indexMethodsByInstanceId = function( methodsArr ) {
				return _.indexBy( methodsArr || [], function( m ) {
					return String( m.instance_id );
				} );
			},
			...
			ShippingMethod       = Backbone.Model.extend({
				...
				logChanges: function( changedRows ) {
					...
					if ( changedRows.methods ) {
					_.each( changedRows.methods, function( row, id ) {
						// Flat map instance_id -> row (do not nest a bogus `methods` key — breaks _.each / save UX).
						changes.methods = changes.methods || {};
			...
			ShippingMethodView = Backbone.View.extend({
				...
				initialize: function() {
					...
					$( document.body ).on( 'click', '.wc-shipping-zone-method-delete', { view: this }, this.onDeleteRow );
					...
					// Capture-phase listener so clicks can't be swallowed by other handlers.
					if ( ! this._epToggleCaptureBound && $tbody.get( 0 ) ) {
						this._epToggleCaptureBound = true;
						$tbody.get( 0 ).addEventListener(
							'click',
							function( event ) {
								var del = event.target && event.target.closest
									? event.target.closest( '.wc-shipping-zone-method-delete' )
									: null;
								...
								if ( del ) {
									...
									view.onDeleteRow.call( del, event );
									return;
								}

								var toggle = event.target && event.target.closest
									? event.target.closest( '.woocommerce-input-toggle' )
									: null;
								...
								if ( ! toggle.closest( '.wc-shipping-zone-method-enabled' ) ) {
									return;
								}
				...
				render: function() {
					...
					if ( _.size( methods ) ) {
						...
						// Toggle click is handled via capture-phase listener in initialize().
					} else {
				...
				onDeleteRow: function( event ) {
					var view    = event.data.view,
						model   = view.model,
						methods   = indexMethodsByInstanceId( model.get( 'methods' ) ),
						changes = {},
						instance_id = String( $( this ).closest('tr').attr('data-id') );
					...
					changes.methods = {};
					changes.methods[ instance_id ] = { deleted : 'deleted', instance_id : instance_id };
					model.set( 'methods', _.values( methods ) );
					model.logChanges( changes );
					view.render();
				},
				onToggleEnabled: function( event ) {
					var view        = event.data.view,
						...
						model       = view.model,
						methods     = indexMethodsByInstanceId( model.get( 'methods' ) ),
						instance_id = String( $tr.attr( 'data-id' ) ),
						enabled     = $tr.attr( 'data-enabled' ) === 'yes' ? 'no' : 'yes',
						changes     = {};

					event.preventDefault();
					event.stopImmediatePropagation();

					if ( ! methods[ instance_id ] ) {
						return;
					}
					methods[ instance_id ].enabled = enabled;
					changes.methods = {};
					changes.methods[ instance_id ] = { enabled : enabled, instance_id : instance_id };
					model.set( 'methods', _.values( methods ) );
					model.logChanges( changes );
					view.render();
				},
				setUnloadConfirmation: function() {
					...
					$( '.ep-easyparcel-shipping-zone-method-save' ).prop( 'disabled', false );
				},
				clearUnloadConfirmation: function() {
					this.needsUnloadConfirm = false;
					$( '.ep-easyparcel-shipping-zone-method-save' ).prop( 'disabled', true );
				},
				...
				updateModelOnChange: function( event ) {
					var model     = event.data.view.model,
						$target   = $( event.target ),
						instance_id   = String( $target.closest( 'tr' ).attr( 'data-id' ) ),
						attribute = $target.data( 'attribute' ),
						value     = $target.val(),
						methods   = indexMethodsByInstanceId( model.get( 'methods' ) ),
						changes = {};
				...
				updateModelOnSort: function( event ) {
					...
					_.each( methods, function( method ) {
						var iid = String( method.instance_id ),
							old_position = parseInt( method.method_order, 10 ),
							new_position = parseInt( $table.find( 'tr[data-id="' + iid + '"]' ).index() + 1, 10 );

						if ( old_position !== new_position ) {
							methods[ iid ].method_order = new_position;
							changes.methods = changes.methods || {};
							changes.methods[ iid ] = _.extend(
								changes.methods[ iid ] || {}, { method_order : new_position, instance_id : iid }
							);
						}
					} );

					if ( changes.methods && _.size( changes.methods ) ) {
						model.set( 'methods', _.values( methods ) );
						model.logChanges( changes );
					}
				},
				onConfigureShippingMethod: function( event ) {
					...
					var instance_id = String( $( this ).closest( 'tr' ).attr( 'data-id' ) ),
